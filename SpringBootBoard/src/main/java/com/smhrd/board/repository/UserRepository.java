...
@Repository// 어노테이셔 ㄴ안써도 됨 --> extends JpaRepository를 해주어서!
public interface UserRepository extends JpaRepository<UserEntity, Long>{
	...
	//ID : entity가 가지고 이ㅛ는 pk필드의 타입 (@Id가 걸려있는 필드의 타입)
	...
	//5. 커스텀 메소드
	//에시) select * from 테이블명 where userId = ? and pw = ?
	/*
	 ...
	 * 
	 * 6. existㄴBy컬럼명(매개변수) :데이터 유무를 알려주는 함수
	 * --> select count ~ limit
	...
	// user_id 를 바탕으로 데이터가 존재하는지 유무를 알려주는 기능
	
	
	// 로그인 기능
	//npe(null pointer exception)를 피하기 위해 사용
	//null이 왔을 시 프로그램 종료의 위험을 막기 위해  -> 값이 없을 시 empty를 retrun
